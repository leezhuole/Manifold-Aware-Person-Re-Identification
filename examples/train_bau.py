...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch+1 in args.eval_epochs) or (epoch == args.epochs-1):
            ...
            if wandb is not None and getattr(wandb, "run", None) is not None:
                ...
                try:
                    wandb.log({'eval/mAP': mAP, 'epoch': epoch})
                except Exception:
                    pass
            if mAP > best_mAP:
                best_mAP = mAP
    ...
    if wandb is not None and getattr(wandb, "run", None) is not None:
        try:
            if isinstance(final_results, tuple):  # (cmc_scores, mAP)
                cmc_scores, final_mAP = final_results
                for rank, score in enumerate(cmc_scores, start=1):
                    wandb.log({f'eval/CMC_top_{rank}': score, 'final/mAP': final_mAP})
            else:
                wandb.log({'final/mAP': final_results})
            wandb.finish()
        except Exception:
            pass
